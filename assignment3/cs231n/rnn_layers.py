...
def rnn_step_backward(dnext_h, cache):
    ...
    dtanh = (1 - np.tanh(x.dot(Wx) + prev_h.dot(Wh) + b) ** 2) * dnext_h
    dx = dtanh.dot(Wx.T)
    dWx = x.T.dot(dtanh)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    h = np.zeros(shape=(x.shape[0], x.shape[1], h0.shape[1]))
    cur_h = h0
    cache = []
    for step in range(x.shape[1]):
        cur_input = x[:,step, :]
        h[:, step, :], new_cache = rnn_step_forward(cur_input, cur_h, Wx, Wh, b)
        cache.append(new_cache)
        cur_h = h[:, step, :]
    
    cache = np.array(cache, dtype=object)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def rnn_backward(dh, cache):
    ...
    for step in range(num_steps - 1, -1, -1):    
        ...
        dx[:, step, :], dnext_h, dcur_Wx, dcur_Wh, dcur_b = rnn_step_backward(dnext_h + dh[:, step, :], cache[step])
        dnext_h
        dWx += dcur_Wx
        dWh += dcur_Wh
        db += dcur_b
