...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        tgt = self.positional_encoding(caption_embed)
        memory = self.visual_projection(features).unsqueeze(1)
        tgt_mask = torch.tril(torch.ones(T, T, device=tgt.device, dtype=tgt.dtype))
        Y = self.transformer(tgt, memory, tgt_mask=tgt_mask)
