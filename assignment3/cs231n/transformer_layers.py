...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        Q = self.query(query).view(N, S, H, HD).moveaxis(1, 2)
        K = self.key(key).view(N, T, H, HD).moveaxis(1, 2)
        V = self.value(value).view(N, T, H, HD).moveaxis(1, 2)
        
        alpha = Q @ K.transpose(2, 3) / HD
         
        if attn_mask is not None: 
              alpha = alpha.masked_fill(attn_mask==0, float("-inf"))
        
        Y = self.attn_drop(F.softmax(alpha, dim=3)) @ V
